    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('post')
                ...
                ->modalDescription('Posting will book the Accounts Payable journal entry for this invoice. This cannot be undone.')
                ->action(function () {
                    try {
                        app(SupplierInvoicePostingService::class)->post($this->record);
                        $this->record->refresh();
                        $this->fillForm();

                        Notification::make()->title('Supplier Invoice posted')->success()->send();
