    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('post')
                ...
                ->modalDescription('Posting will book the cash receipt and update the applied invoice\'s paid amount. This cannot be undone.')
                ->action(function () {
                    try {
                        app(IncomingPaymentPostingService::class)->post($this->record);
                        $this->record->refresh();
                        $this->fillForm();

                        Notification::make()->title('Incoming Payment posted')->success()->send();
